...
@media (max-width: 480px) {
    ...
    // Header only (no onboarding/registration)
    ...
    launchButton.innerHTML = `
        ...
        <span class="chat-launcher-text">Ask me</span>`;
    ...
    // First-visit tooltip
    const TOOLTIP_KEY = 'n8n_chat_widget2_tooltip_dismissed';
    const tooltip = document.createElement('div');
    tooltip.style.position = 'fixed';
    tooltip.style.bottom = '86px';
    tooltip.style[settings.style.position === 'left' ? 'left' : 'right'] = '20px';
    tooltip.style.zIndex = '1001';
    tooltip.style.background = '#ffffff';
    tooltip.style.color = 'var(--chat-widget-text, #1f2937)';
    tooltip.style.border = '1px solid var(--chat-widget-border, #e5e7eb)';
    tooltip.style.boxShadow = '0 10px 15px rgba(0,0,0,0.1)';
    ...
    function showTooltipIfFirstVisit() {
        try {
            const dismissed = localStorage.getItem(TOOLTIP_KEY) === 'true';
            if (!dismissed) {
    ...
    const tooltipCloseBtn = tooltip.querySelector('button');
    if (tooltipCloseBtn) {
        tooltipCloseBtn.addEventListener('click', () => {
            tooltip.style.display = 'none';
            try { localStorage.setItem(TOOLTIP_KEY, 'true'); } catch (_) {}
        });
    }
    ...
    // No onboarding: ensure chat body is visible when widget opens
    function showChat() {
    ...
    // Show tooltip shortly after load
    setTimeout(showTooltipIfFirstVisit, 600);
})();